  ...
  app.innerHTML = `
    ...
    <article class="question-card">
      ...
      <div class="action-row">
        ...
        ${answered && session.mode !== "wrong" ? manualWrongButton(question.id) : ""}
...
function manualWrongButton(id) {
  const alreadyAdded = state.wrongIds.includes(id);
  return `
    <button class="submit-btn" type="button" data-action="manual-wrong" ${alreadyAdded ? "disabled" : ""}>
      ${alreadyAdded ? "已加入错题集" : "犹豫了，手动添加到错题集"}
    </button>
  `;
}
...
document.addEventListener("click", (event) => {
  ...
  if (actionButton) {
    ...
    if (action === "manual-wrong") addCurrentQuestionToWrongSet();
...
function addCurrentQuestionToWrongSet() {
  if (!session || !answered || session.mode === "wrong") return;
  const question = questionById(session.ids[session.cursor]);
